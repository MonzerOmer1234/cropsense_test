...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="card-style mb-30">
                        <form id="farmForm" action="{{ route('farms.update', $farm->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-lg-5">
                                    <div class="input-style-1">
                                        <label>{{ __('Name') }}</label>
                                        <input type="text" placeholder="{{ __('Name') }}" required name="name" value="{{ $farm->name }}" />
                                    </div>
                                    <div class="input-style-1">
                                        <label>{{ __('Location') }}</label>
                                        <input type="text" placeholder="{{ __('Location') }}" required name="location" value="{{ $farm->location }}" />
                                        @error('location')
                                    ...
                                    <div class="input-style-1">
                                        <label>{{ __('Size') }}</label>
                                        <input type="text" placeholder="{{ __('Size') }}" required name="size" value="{{ $farm->size }}" />
                                        @error('size')
                                            <div class="text-danger">
                                                {{ $message }}
                                            </div>
                                    ...
                                    <div class="input-style-1">
                                        ...
                                        <input type="text" placeholder="{{ __('Crop Type') }}" required name="crop_type" value="{{ $farm->crop_type }}" />
                                        @error('crop_type')
                                            <div class="text-danger">
                                                {{ $message }}
                                            </div>
                                    ...
                                    <div class="input-style-1">
                                        ...
                                        <textarea placeholder="{{ __('Description') }}" rows="5" name="description">{{ $farm->description }}</textarea>
                                    </div>
                                </div>
                                {{-- Location  Map  --}}
                                <div class="col-lg-7">
                                    <div class="card-style mb-30">
                                        ...
                                        <input type="hidden" id="points" name="points" value="{{ $farm->points }}">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var existingPoints = JSON.parse(document.getElementById('points').value || '[]');
        if (existingPoints && existingPoints.length > 0) {
            var polygon = L.polygon(existingPoints).addTo(drawnItems);
